...
def get_sheet():
    ...
    creds = Credentials.from_service_account_info(
        ...
        scopes=[
            ...
            "https://www.googleapis.com/auth/spreadsheets",  # ✅ เพิ่ม scope
        ]
    )
    gc = gspread.Client(auth=creds)  # ✅ แก้จาก gspread.authorize() ที่ deprecated แล้ว
...
def parse_with_claude(text: str) -> dict:
    resp = claude.messages.create(
        model="claude-haiku-4-5-20251001",  # ✅ แก้ชื่อ model ให้ถูกต้อง
        max_tokens=300,
        system=SYSTEM_PROMPT,
        messages=[{"role": "user", "content": text}]
    )
    raw = resp.content[0].text.strip()
    raw = raw.replace("```json", "").replace("```", "").strip()  # ✅ กัน markdown จาก response
...
def handle_image(event):
    try:
        ...
        image_bytes = msg_content.content  # ✅ แก้จาก iter_content() ที่ไม่มีใน LINE SDK
